...
def train_and_fit(args):
    ...
    print("NUM CLASSES: %d\n%s" % (num_classes, str(labels_dict['l2idx'].keys())))

    tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')

    net = DistilBertForSequenceClassification.from_pretrained('distilbert-base-uncased', num_labels=num_classes)
    print("VOCAB SIZE: %d" % len(tokenizer))
    ...
    logger.info("FREEZING MOST HIDDEN LAYERS...")
    unfrozen_layers = ["classifier",  "pre_classifier", "pooler", "distilbert.transformer.layer.5",\
                        #"distilbert.transformer.layer.4", # "distilbert.transformer.layer.4.ffn" "distilbert.transformer.layer.4.output_layer_norm",
                        ]
    for name, param in net.named_parameters():
    ...
    optimizer = optim.Adam([ {"params":net.classifier.parameters(), "lr": args.lr},
                             {"params":net.pre_classifier.parameters(), "lr": args.lr}])
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            inputs, _, mask, labels = data
            if cuda:
                inputs, mask, labels = inputs.cuda(), mask.cuda(), labels.cuda()
        ...
        if (epoch % 1) == 0:
            save_as_pickle(os.path.join(args.train_data, "test_losses_per_epoch_%d.pkl" % args.model_no), losses_per_epoch)
            save_as_pickle(os.path.join(args.train_data, "test_accuracy_per_epoch_%d.pkl" % args.model_no), f1_per_epoch)
            if args.use_sagemaker != 1:
                torch.save({
                        'epoch': epoch + 1,\
                        'state_dict': net.state_dict(),\
