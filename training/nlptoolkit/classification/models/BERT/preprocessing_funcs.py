...
def preprocess(args):
    ...
    tokenizer = BertTokenizer.from_pretrained(args.model_size, do_lower_case=True)
    special_tokens_dict = {'additional_special_tokens': ['[NAME]','[PLACE]','[NUMBER]','[TAG]', '[DATE]']}
    num_added_toks = tokenizer.add_special_tokens(special_tokens_dict)
    tokens_length = args.tokens_length # max tokens length
    
    logger.info("Tokenizing data...")
    ### tokenize data for BERT
    df_train.loc[:, "text"] = df_train["text"].progress_apply(lambda x: tokenizer.tokenize("[CLS] " + x))
    df_train.loc[:, "text"] = df_train["text"].progress_apply(lambda x: tokenizer.convert_tokens_to_ids(x[:(tokens_length-1)] + ["[SEP]"]))
