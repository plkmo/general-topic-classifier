...
def train_and_fit(args):
    ...
    try:
        tokenizer = load_pickle(os.path.join(args.train_data, 'tokenizer.pkl'))
    except Exception as e:
        print("Loading tokenizer from /opt/ ")
        with open("/opt/ml/model/tokenizer.pkl", 'rb') as pkl_file:
            tokenizer = pickle.load(pkl_file)

    net = DebertaForSequenceClassification.from_pretrained('microsoft/deberta-base', num_labels=num_classes)
    if cuda:
        net.cuda()
    ...
    logger.info("FREEZING MOST HIDDEN LAYERS...")
    unfrozen_layers = ["classifier", "pooler", "deberta.encoder.rel_embeddings", "deberta.encoder.layer.11",\
                        "deberta.encoder.LayerNorm.weight", "deberta.encoder.conv.conv", "deberta.encoder.conv.LayerNorm.weight",\
                        ]
    for name, param in net.named_parameters():
    ...
    optimizer = optim.Adam([{"params":net.deberta.parameters(),"lr": 0.0003},\
